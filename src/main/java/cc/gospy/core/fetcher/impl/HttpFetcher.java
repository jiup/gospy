...
public class HttpFetcher implements Fetcher {
    ...
    public static void main(String[] args) throws Throwable {
        Fetcher fetcher = HttpFetcher.custom().beforeRequest(request -> {
            request.setHeader("User-Agent", "Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10_6_8; en-us) AppleWebKit/534.50 (KHTML, like Gecko) Version/5.1 Safari/534.50");
            request.setConfig(RequestConfig.custom().setRedirectsEnabled(false).build());
        }).setCleanPeriodSeconds(5).setAutoKeepAlive(false).build();
        List<Page> list = new ArrayList<>();
        list.add(fetcher.fetch(new Task("http://www.12306.cn/mormhweb/")));
        list.add(fetcher.fetch(new Task("http://www.zhangjiupeng.com/")));
        list.add(fetcher.fetch(new Task("https://www.baidu.com")));
    ...
    public Page fetch(Task task) throws Throwable {
        ...
        if (extra != null) {
            response = doPost(task.getUrl(), task.getExtra());
        } else {
            response = doGet(task.getUrl());
        }
